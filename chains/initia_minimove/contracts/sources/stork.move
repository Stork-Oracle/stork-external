module stork::stork {
    ...
    entry fun init_stork(
        ...
        let single_update_fee = fee::new(single_update_fee_amount, single_update_fee_denom);
    ...
    fun transfer_fee(
        ...
        // coin::deposit<AptosCoin>(state_object_store::get_state_object_address(), coin);
        let fee_metadata_object = coin::denom_to_metadata(fee.get_denom());
        coin::transfer(signer, state_object_store::get_state_object_address(), fee_metadata_object, fee.get_amount() * multiplier);
    }
    ...
    #[test_only]
    use initia_std::account::{Self, create_account_for_test};
    #[test_only]
    use std::vector;
    #[test_only]
    ...
    #[test_only]
    const COIN_DENOM_BYTES: vector<u8> = b"uinit";
    ...
    fun setup_test(): (signer, signer) {
        ...
        let fee_amount = 1;
        let fee_denom = string::utf8(COIN_DENOM_BYTES);

        let user_signer = create_account_for_test(USER);

        // test coin initialization
        let std_signer = account::create_account_for_test(@initia_std);
        let (mint_cap, _burn_cap, _freeze_cap, _extend_ref) = coin::initialize_and_generate_extend_ref(
            &std_signer,
            std::option::none(),
            string::utf8(b""),
            fee_denom,
        ...
        coin::mint_to(&mint_cap, USER, 50); 
        coin::mint_to(&mint_cap, DEPLOYER, 50);
    ...
    fun test_init_stork_already_initialized() {
        ...
        let fee_amount = 1;
        let fee_denom = string::utf8(COIN_DENOM_BYTES);
        init_stork(&deployer_signer, x"0a803F9b1CCe32e2773e0d2e98b37E0775cA5d44", fee_amount, fee_denom);
    }
    ...
    fun test_fee_transfer() {
        ...
        let initial_user_balance = coin::balance(USER, fee_metadata_object);

        let initial_state_account_balance = coin::balance(state_object_store::get_state_object_address(), fee_metadata_object);

        transfer_fee(&user_signer, fee, 1);

        assert!(coin::balance(USER, fee_metadata_object) == initial_user_balance - fee.get_amount(), 0);
        assert!(coin::balance(state_object_store::get_state_object_address(), fee_metadata_object) == initial_state_account_balance + fee.get_amount(), 1);
    }
