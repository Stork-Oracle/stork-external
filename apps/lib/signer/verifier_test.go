...
func TestEvmVerifier_VerifyPublisherPrice(t *testing.T) {
	...
	err := VerifyEvmPublisherPrice(1710191092123456789, "BTCUSDMARK", "72147681412670819000000", pubKey, signature)
	if err != nil {
		t.Error(err)
	}

	// change the price slightly
	err = VerifyEvmPublisherPrice(1710191092123456789, "BTCUSDMARK", "72147681412670719000000", pubKey, signature)
	if err == nil {
		t.Fail()
	}
}
...
func TestStarkVerifier_VerifyPublisherPrice(t *testing.T) {
	...
	err := VerifyStarkPublisherPrice(1708940577123456789, "0x44594458555344000000000000000000637a6f7778", "3335950349880000000", pubKey, signature)
	if err != nil {
		t.Error(err)
	}

	// change the price slightly
	err = VerifyStarkPublisherPrice(1708940577123456789, "0x44594458555344000000000000000000637a6f7778", "3335950348880000000", pubKey, signature)
	if err == nil {
		t.Fail()
	}
}
