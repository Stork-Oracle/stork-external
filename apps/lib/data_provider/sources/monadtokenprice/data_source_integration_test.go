...
func TestMonadTokenPriceDataSource_RunDataSource(t *testing.T) {
	config := types.DataProviderSourceConfig{
		Id: "MY_TEST_VALUE_ID",
		Config: MonadTokenPriceConfig{
			DataSource:      MonadTokenPriceDataSourceId,
			// TODO: add valid configuration
		},
	}

	dataSource := newMonadTokenPriceDataSource(config)
