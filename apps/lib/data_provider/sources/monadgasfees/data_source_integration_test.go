...
func TestMonadGasFeesDataSource_RunDataSource(t *testing.T) {
	config := types.DataProviderSourceConfig{
		Id: "MY_TEST_VALUE_ID",
		Config: MonadGasFeesConfig{
			DataSource:      MonadGasFeesDataSourceId,
			// TODO: add valid configuration
		},
	}

	dataSource := newMonadGasFeesDataSource(config)
