...
func TestPriceUpdateToTemporalNumericValueEvmInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rBytes, _ := pusher.HexStringToByteArray(tt.priceUpdate.StorkSignedPrice.TimestampedSignature.Signature.R)
			sBytes, _ := pusher.HexStringToByteArray(tt.priceUpdate.StorkSignedPrice.TimestampedSignature.Signature.S)
