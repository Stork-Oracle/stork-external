module stork::state_object_store {
    ...
    /// Runs on publish, sets up the state object store
    fun init_module(package: &signer){
        let state_object_constructor_ref = object::create_named_object(package, STATE_OBJECT_SEED);
        let state_object_extend_ref = object::generate_extend_ref(&state_object_constructor_ref);
        let state_object_store = new(state_object_extend_ref);
