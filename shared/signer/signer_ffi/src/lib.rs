...
pub extern "C" fn hash_and_sign(
    ...
    let x_fe = bytes_to_field_element(x_ptr)
        .unwrap_or_else(|_e| panic!("Failed to convert x byte buffer to field element"));
    let y_fe = bytes_to_field_element(y_ptr)
        .unwrap_or_else(|_e| panic!("Failed to convert y byte buffer to field element"));
    let pk_fe = bytes_to_field_element(pk_ptr)
        .unwrap_or_else(|_e| panic!("Failed to convert pk byte buffer to field element"));

    let hashed = pedersen_hash(&x_fe, &y_fe);
    let signature = starknet_core::crypto::ecdsa_sign(&pk_fe, &hashed)
        .unwrap_or_else(|_e| panic!("Failed to sign pedersen hash"));
...
#[no_mangle]
pub extern "C" fn validate_stark_signature(
    x_ptr: *const u8,
    y_ptr: *const u8,
